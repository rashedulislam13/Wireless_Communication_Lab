...
state = zeros(1, m); % Shift register initialized to zeros
...
for i = 1:length(input_to_Convolutional_encoder)
    ...
    % Ensure state and generator have compatible lengths
    % Output bits: XOR (mod 2) of state with generators
    out1 = mod(sum(state .* g1(2:end)) + input_bit * g1(1), 2); % Include current bit
...
% 4-QAM Modulation (replacing qammod)
M = 4;  % 4-QAM (equivalent to QPSK)
...
num_symbols = length(data_interleave) / k;
symbol = zeros(num_symbols, 1);
for i = 1:num_symbols
    bit_pair = data_interleave((i-1)*k + 1:i*k);
    symbol(i) = bit_pair(1) * 2 + bit_pair(2); % 00->0, 01->1, 10->2, 11->3
end
% 4-QAM constellation: [1+1j, -1+1j, -1-1j, 1-1j] / sqrt(2)
